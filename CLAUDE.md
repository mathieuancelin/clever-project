...
- **Cross-resource refs** come in three shapes: `${apps.KEY.env.VAR}`, `${addons.KEY.env.VAR}`, and `${addons.KEY.addon.dot.path}`. None are resolved at load time — `Resolver::resolve_string_inner` detects them via `parse_cross_ref` (returns a `CrossRef` enum: `AppEnv` / `AddonEnv` / `AddonMeta`) and echoes the literal `${...}` back. The second-pass resolver lives in `commands/cross_refs.rs::resolve_in_project`. It scans `project.apps.*.env` and `project.display`, fetches each unique source once (`Clever::get_env_full` for app env, `Clever::get_addon_env` for addon env, `Clever::get_addon_meta` for addon metadata), and substitutes in place. The metadata fetch hits `/v4/addon-providers/{addon-X}/addons/{realId}` — note the `addon-` prefix; `Clever::normalize_v4_provider_id` adds it when the v2 provider id is bare (e.g. `otoroshi` → `addon-otoroshi`). Addon-meta paths walk the JSON tree; terminal must be a scalar (`String` / `Number` / `Bool`) — objects/arrays return `None` and surface as "not found" warnings. Missing data on any shape → warning + empty substitution, not a hard error.
